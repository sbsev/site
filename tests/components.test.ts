...
test.describe('Component Functionality', () => {
  ...
  test('modal/dialog functionality', async ({ page }) => {
    ...
    const modalTriggers = page.locator('button:has-text("info"), button:has-text("mehr"), button[aria-haspopup="dialog"]')
    ...
    if (count > 0) {
      ...
      if (await modal.count() > 0) {
        ...
        const closeButton = page.locator('button:has-text("×"), button:has-text("close"), button[aria-label*="close"]')
  ...
  test('dropdown menus work', async ({ page }) => {
    ...
    const dropdownTriggers = page.locator('button:has-text("öffnen"), button[aria-expanded], [class*="dropdown"] button')
  ...
  test('pagination works', async ({ page }) => {
    ...
    if (await pagination.count() > 0) {
      const nextButton = page.locator('a:has-text("next"), a:has-text("weiter"), a:has-text(">")')
  ...
  test('social media links work', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(count, 3); i++) {
      ...
      expect(href).toMatch(/^https?:\/\/(www\.)?(facebook|twitter|instagram|linkedin)\.com/)
