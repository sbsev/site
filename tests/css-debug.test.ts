...
test('debug CSS loading in head section', async ({ page }) => {
  ...
  const themeCssLinks = await page.locator('head link[href*="theme.css"]').count()
  console.log('Number of theme CSS links found:', themeCssLinks)
  
  if (themeCssLinks > 0) {
    const href = await page.locator('head link[href*="theme.css"]').first().getAttribute('href')
    console.log('Theme CSS href:', href)
  }
  
  // Check for app.css
  const appCssLinks = await page.locator('head link[href*="app.css"], head style').count()
